...
void ScrapEngine::Render::UniformBuffer::update_uniform_buffer(const uint32_t& current_image,
                                                               const ScrapEngine::Core::STransform& object_transform,
                                                               ScrapEngine::Render::Camera* render_camera)
{
	UniformBufferObject ubo = {};

	ubo.model = glm::translate(glm::mat4(1.0f), object_transform.get_position().get_glm_vector());
	ubo.model = glm::scale(ubo.model, object_transform.get_scale().get_glm_vector());

	ubo.model = glm::rotate(ubo.model, glm::radians(object_transform.get_rotation().get_x()), glm::vec3(1.0f, 0.0f, 0.0f));
	ubo.model = glm::rotate(ubo.model, glm::radians(object_transform.get_rotation().get_y()), glm::vec3(0.0f, 1.0f, 0.0f));
	ubo.model = glm::rotate(ubo.model, glm::radians(object_transform.get_rotation().get_z()), glm::vec3(0.0f, 0.0f, 1.0f));
	...
	ubo.view = glm::lookAt(render_camera->get_camera_location().get_glm_vector(),
