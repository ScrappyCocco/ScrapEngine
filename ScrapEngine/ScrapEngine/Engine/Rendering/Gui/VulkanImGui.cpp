...
void ScrapEngine::Render::VulkanImGui::init_resources(VulkanSwapChain* swap_chain, BaseRenderPass* render_pass)
{
	...
	//Prepare buffer
	ImageStagingBuffer* staging_buffer_ref = new ImageStagingBuffer(upload_size, font_data);
	...
	//Copy
	ImageStagingBuffer::copy_buffer_to_image(staging_buffer_ref->get_staging_buffer(), &font_image_,
	                                         &buffer_copy_region, 1, vk::ImageLayout::eTransferDstOptimal);
	delete staging_buffer_ref;
	staging_buffer_ref = nullptr;
